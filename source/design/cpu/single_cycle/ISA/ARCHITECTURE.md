...
The top-level module is `sc_cpu`, responsible for connecting all functional units and updating architectural state.

### Main Modules
| Module | Function |
|-----------------|-----------------|
| **sc_cpu** | Integrates datapath and control path, updates PC, exports halt status |
...
3. **Control**: `control_unit` generates primary control signals from instruction fields (`opcode`, `funct`).
...
### Branch Policy
In this implementation, branch instruction identity (`is_beq`, `is_bne`, `is_blt`) is generated by `control_unit`, while branch decision (`take_branch`) is generated in `sc_cpu` using ALU flags:
...
### System
- `HALT` (stops PC update and raises `halt` output)
...
- Existing project implementation in Verilog (`sc_cpu`, `control_unit`, `alu`, `register_file`, `instr_mem`, `data_mem`).
